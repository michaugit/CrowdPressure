...
/** The class doing geometrical maths calculations */
public class GeometricCalculator {
    public static final BiFunction<LinePointAngle, LineTwoPoints, Optional<Position>> crossPointTwoLines = (
        ...
        // TerminalPrinter.print("ANGLE: " + lpa.getAngle() + " wall: POS: " + Arrays.asList(ltp, new
        // Position(x, y)));

        if (checkIfPointIsInBetween(new Position(x, y), new Position(xp1, yp1), new Position(xp2, yp2))) {
            // TerminalPrinter.print("HERE CROST POINT: " + lpa.getAngle() + new Position(x, y));
    ...
    public static BiFunction<Position, Position, Double> calculateAngle = (Position pos, Position destinationPoint) -> {
        // TerminalPrinter.print("POSITIONS: " + Arrays.asList(pos, destinationPoint));
        double result = (pos.getY() - destinationPoint.getY()) / (pos.getX() - destinationPoint.getX());
        // TerminalPrinter.print("calcAng: " + result);
    ...
    public static BiFunction<Position, LineTwoPoints, Optional<Vector>> vectorStraightPoint = (Position p,
        ...
        // TerminalPrinter.print("poi: " + Arrays.asList(p,l));
        // TerminalPrinter.print("xy: " + Arrays.asList(x,y));
    ...
    public static final VectorXY changeVector(Vector v) throws AngleOutOfRangeException {
        ...
        // TerminalPrinter.print("CH vector:",v);
    ...
    public static boolean checkIfPointIsInBetween(Position checking, Position pA, Position pB) {
        ...
        return (((x1 - DefaultConfig.PRECISION_OF_CALCULATIONS <= x
                && x <= x2 + DefaultConfig.PRECISION_OF_CALCULATIONS)
                || (x2 - DefaultConfig.PRECISION_OF_CALCULATIONS <= x
                && x <= x1 + DefaultConfig.PRECISION_OF_CALCULATIONS))
                && ((y1 - DefaultConfig.PRECISION_OF_CALCULATIONS <= y
                && y <= y2 + DefaultConfig.PRECISION_OF_CALCULATIONS)
                || (y2 - DefaultConfig.PRECISION_OF_CALCULATIONS <= y
                && y <= y1 + DefaultConfig.PRECISION_OF_CALCULATIONS)));
    }

    public static VectorXY subtractVectors(VectorXY v1, VectorXY v2) {
        // TerminalPrinter.print("SUBTRACT " + Arrays.asList(v1, v2));
    ...
    public static Vector subtractVectors(Vector v1, Vector v2) throws AngleOutOfRangeException {
        // TerminalPrinter.print(Arrays.asList(v1,v2));
    ...
    public static Vector addVectors(Vector v1, Vector v2) throws AngleOutOfRangeException {
        // TerminalPrinter.print("addVectors: " + Arrays.asList(v1,v2));
